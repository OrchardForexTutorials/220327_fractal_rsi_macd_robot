...
public:
   CExpert( CIndicatorMACD *macd, CIndicatorFractal *fractal, //
            double tpslRatio,                                 //
...
//
CExpert::CExpert( CIndicatorMACD *macd, CIndicatorFractal *fractal, //
                  double tpslRatio,                                 //
                  double volume, string tradeComment, int magic )
...
void CExpert::Loop() {
   ...
   double main       = mMACD.GetData( MAIN_LINE, 1 );
   double signal     = mMACD.GetData( SIGNAL_LINE, 1 );
   ...
   if ( mBuyPrice > 0 && closePrice > mBuyPrice && main > signal ) {
      OpenTrade( ORDER_TYPE_BUY, mBuyPrice - mBuySL );
      mBuyPrice = 0;
   }
   else if ( mSellPrice > 0 && closePrice < mSellPrice && main < signal ) {
